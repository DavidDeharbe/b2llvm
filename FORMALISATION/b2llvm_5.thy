...
datatype LStm =
  ...
  BrC LType LExpr LLabel LLabel |    -- "Conditional branch"
  Label LLabel
...
*)          
fun b2llvm_pred :: "BContext \<Rightarrow> VarLayout \<Rightarrow> BPredicate \<Rightarrow> LLabel \<Rightarrow> LLabel \<Rightarrow> LLabel \<Rightarrow> LTemp \<Rightarrow> (LStm list * LExpr * LLabel * LTemp)"
where
  "b2llvm_pred \<Gamma> \<L> \<phi> label_true label_false nlab ntemp =
    (case \<phi> of
    
      (BEq e1 e2) \<Rightarrow> 
        (case (b2llvm_expr \<Gamma> \<L> e1 ntemp) of
          (il1, v1, ty1, ntemp1) \<Rightarrow>
            (case (b2llvm_expr \<Gamma> \<L> e2 ntemp1) of
              (il2, v2, ty2, ntemp2) \<Rightarrow> 
                ( il1 @ il2 @ 
                  [ ICmpEq ntemp2 ty1 v1 v2 ,
                    BrC (LIntType 1) (Var ntemp2) label_true label_false], 
                  Var ntemp2, nlab, ntemp2 + 1))) |
                  
      (BConj p1 p2) \<Rightarrow>
        (case (b2llvm_pred \<Gamma> \<L> p1 nlab label_false (nlab + 1) ntemp) of
          (il1, v1, nlab1, ntemp1) \<Rightarrow>
            (case (b2llvm_pred \<Gamma> \<L> p2 label_true label_false nlab1 ntemp1) of
              (il2, v2, nlab2, ntemp2) \<Rightarrow> 
                (il1 @ 
                 [ BrC (LIntType 1) v1 nlab label_false, 
                   Label nlab ] @ 
                 il2, 
                 v2, nlab2, ntemp2))) |

      (BNeg p) \<Rightarrow>
        (case (b2llvm_pred \<Gamma> \<L> p label_false label_true nlab ntemp) of
          (il, v, nlab', ntemp') \<Rightarrow> (il, v, nlab', ntemp')))"
...
fun b2llvm_stm :: "BContext \<Rightarrow> VarLayout \<Rightarrow> BInst \<Rightarrow> LLabel \<Rightarrow> LTemp \<Rightarrow> (LStm list * LLabel * LTemp)" 
where
  "b2llvm_stm \<Gamma> \<L> s nlab counter =
    (case s of
      Skip \<Rightarrow> ([], nlab, counter) |
      Asg v e \<Rightarrow> 
        (let (sl, e', ty_e, counter') = (b2llvm_expr \<Gamma> \<L> e counter) in
          (let ty_v = b2llvm_type \<Gamma> (\<tau> \<Gamma> v) in
           (sl @ [ Store ty_e e' ty_v (\<L>  v) ], nlab, counter'))) |
      _ \<Rightarrow> ([], nlab, counter))" (* actually should be undefined *)

fun b2llvm_stm_label :: "BContext \<Rightarrow> VarLayout \<Rightarrow> BInst \<Rightarrow> LLabel \<Rightarrow> LLabel \<Rightarrow> LTemp \<Rightarrow> (LStm list * LLabel * LTemp)" 
where
  "b2llvm_stm_label \<Gamma> \<L> s exit label counter = 
    (case s of
      Skip \<Rightarrow>
        (let (sl, label', counter') = b2llvm_stm \<Gamma> \<L> Skip label counter in
          (sl @ [BrU exit], label', counter')) |
      (Asg v e) \<Rightarrow>
        (let (sl, label', counter') = b2llvm_stm \<Gamma> \<L> (Asg v e) label counter in
          (sl @ [BrU exit], label', counter')) |
      (Blk []) \<Rightarrow>
        ( [BrU exit], label, counter ) |
      (Blk (x # xs)) \<Rightarrow>
        (let (sl', label', counter') = 
          (case x of
            (If _ _ _) \<Rightarrow> 
              (let (sli, labeli, counteri) = b2llvm_stm_label \<Gamma> \<L> x label (label + 1) counter in
                (sli @ [ Label label ], labeli, counteri)) |
            _ \<Rightarrow> 
               b2llvm_stm \<Gamma> \<L> x label counter) 
         in    
           (let (sl'', label'', counter'') = b2llvm_stm_label \<Gamma> \<L> (Blk xs) exit label' counter' in
             (sl' @ sl'', label'', counter''))) |
      (If c i1 i2) \<Rightarrow>
        (let (bk_then, bk_else) = (label, label + 1) in
          (let (ilc, e1, counter1, tmp1) = b2llvm_pred \<Gamma> \<L> c bk_then bk_else (label + 2) counter in
            (let (ilt, counter2, tmp2) = b2llvm_stm_label \<Gamma> \<L> i1 exit counter1 tmp1 in
              (let (ile, counter3, tmp3) = b2llvm_stm_label \<Gamma> \<L> i2 exit counter2 tmp2 in
                 (ilc @
                  [Label bk_then] @
                  ilt @
                  [Label bk_else] @
