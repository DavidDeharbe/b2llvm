...
\newcommand{\rst}[0]{\ensuremath{\mathbf{rst}}}
\newcommand{\twodots}[0]{..}
...
\begin{align*}
  \trad{n}{ArrayType} \isdef
  & \LET s \BE \card (\fst(\B{n.dom})) \AND \\
  & \quad et \BE \IF |n.dom| = 1 \THEN \trad{n.ran}{Type} \\
  & \quad \quad \ELSE \trad{\B{(dom = \rst(n.dom), ran = n.ran)}}{ArrayType} \\
  & \IN \llvm{"[ \PH{s} x \PH{et} ]"}
\end{align*}
...
Both B and LLVM include structure types, and the encoding is simple.
...
names in B, and positions starting from 0 in LLVM. 

For example, the record \B{rec(flag: TRUE, balance: 10)} of type
\B{struct(flag: BOOL, balance: INT)} is encoded in LLVM as the
literal \llvm{\{ i1 1, i32 10\}}, of type \llvm{\{ i1, i32 \}}.
